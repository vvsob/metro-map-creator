...
class LineSegment(Element):
    def __init__(self, line: 'Line', line_segment_json: Dict[str, Any], is_planned=None):
...
class Line:
    ...
    def draw(self, metro_map_image):
        ...
        transfer_length = get_transfer(self.line_image, self.type, Orientation.RIGHT).width
        turn_length = get_arc(self.line_image, TurnType.RIGHT_DOWN).width

        for (num, element) in enumerate(self.elements):
            element_image = self.planned_line_image if element.is_actually_planned() else self.line_image
            ...
            if isinstance(element, LineSegment):
                ...
                position = self.continue_line(position, metro_map_image, element_image, line_length, direction)

            if isinstance(element, Station):
                if num == 0:
                    position = self.continue_with_first_station(metro_map_image, element, position, direction,
                                                                element_image)
                elif num != len(self.elements) - 1:
                    position = self.continue_with_station(metro_map_image, element, position, direction, element_image)
                else:
                    self.continue_with_last_station(metro_map_image, element, position, direction, element_image)

            if isinstance(element, Turn):
                position, direction = self.continue_with_turn(metro_map_image, element, position, direction,
                                                              element_image)
    ...
    def draw_logos(self, metro_map_image):
        ...
        if isinstance(first_element, Station):
            ...
            if self.start_logo_offset is not None:
                place(metro_map_image, self.logo_image,
    ...
    def get_linear_metro_map(self, reverse_direction, start_station_name=None, is_bidirectional=False):
        ...
        for (num, station) in enumerate(stations):
            if station.is_transfer():
                ...
                for line in reversed(sorted(station.get_transfer_lines(), key=cmp_to_key(Line.cmp))):
...
class MapData:
    ...
    def draw_info(self, metro_map_image):
        metro_map_image.composite(self.info_image, left=metro_map_image.width - self.info_image.width,
                                  top=metro_map_image.height - self.info_image.height)

    def draw(self, highlighted_station=None):
        metro_map_image = Image(height=self.image_resolution[0], width=self.image_resolution[1],

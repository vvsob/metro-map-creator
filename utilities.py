...
def turn(direction, clockwise):
    ...
    for num, cur_direction in enumerate(directions):
        if cur_direction == direction:
            return (
                directions[(num + 1) % len(directions)]
                if clockwise
                else directions[(num + len(directions) - 1) % len(directions)]
            )
...
def get_text_image(
    ...
    res_image = Image(
        ...
        height=int(
            draw.get_font_metrics(image, text, multiline=True).text_height
            + 2 * padding_size[1]
        ),
...
def round_corners(image, radius):
    ...
    round_corner(
        image, radius, (image.width - 1 - radius, image.height - 1 - radius), True, True
    )
...
def complete_width(image):
    ...
    formatted_image = Image(
        width=total_width, height=128, background=Color("#FFFFFF00")
    )
